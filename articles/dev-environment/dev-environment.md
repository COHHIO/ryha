...
[.devcontainer](https://github.com/COHHIO/ryha/tree/master/.devcontainer)
...
*NOTE*: **Developer: Reload Window** only needs to be run after a
**Rebuild**. There is no need to run it after you **Reopen** a
container.
...
- **Reopen Folder Locally**: This option will close the current Dev
  ...
  environment, outside of the containerized setup. It’s useful when you
  want to switch back to your local development environment while
  keeping the same project open.
- **Close Remote Connection**: This will close the connection to the Dev
  Container and stop the container. The current workspace will be
  closed, and you’ll be returned to the main VS Code window without any
  active workspace or connection.
...
[.devcontainer/Dockerfile.Dev](https://github.com/COHHIO/ryha/tree/master/.devcontainer/Dockerfile.Dev).
This file:
...
[.devcontainer/docker-compose.yml](https://github.com/COHHIO/ryha/tree/master/.devcontainer/docker-compose.yml).
...
[postgres/populate_dev_database/populate_dev_database.R](https://github.com/COHHIO/ryha/tree/master/postgres/populate_dev_database/populate_dev_database.R)
to create and populate the corresponding tables. To run this script you
need to:

- Store `dm.rds` in the directory `postgres/populate_dev_database/data`.
  ...
  list of dataframes where each element is named after the table name
  the data was read from.
- Run `devtools::load_all(".")` to have access to the different
  functions in `ryha` that are used in the script
  (e.g. [`send_to_db()`](https://cohhio.github.io/ryha/reference/send_to_db.md)).
