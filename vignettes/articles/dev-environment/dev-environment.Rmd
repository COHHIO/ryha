...
- [complete_dev_setup.R](https://github.com/COHHIO/ryha/tree/master/.devcontainer/complete_dev_setup.R) is a script run as a `postCreateCommand`. It performs the following tasks:
  - enables `pak` as the `renv` backend (for automatic system dependency resolution) and runs `renv::restore()` to restore the project's app packages from the [renv.lock](https://github.com/COHHIO/ryha/tree/master/renv.lock) file
